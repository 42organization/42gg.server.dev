...
public interface PChangeRepository extends JpaRepository<PChange, Integer> {
    ...
    @Query(nativeQuery = true, value = "SELECT * FROM pchange where game_id in (SELECT id FROM game where id < :gameId AND mode = :mode) AND user_id = :intraId ORDER BY id Desc"
